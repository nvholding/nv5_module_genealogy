...
foreach( $global_array_fam as $fid_i => $array_value )
{
	...
	elseif( isset( $array_fam_admin[$admin_id][$fid_i] ) )
	{
		...
		elseif( $_fam_admin_i['add_content'] == 1 )
		{
			$check_fam = true;
		}
		elseif( $_fam_admin_i['pub_content'] == 1 or $_fam_admin_i['app_content'] == 1 )
		{
			$check_fam = true;
			$check_declined = true;
		}
		elseif( $_fam_admin_i['edit_content'] == 1 )
		{
			$check_fam = true;
		}
		elseif( $_fam_admin_i['del_content'] == 1 )
		{
			$check_fam = true;
		}
	}
	...
	if( $check_fam )
	{
		...
		$array_fam_view[] = $fid_i;
	}
}
if( ! defined( 'NV_IS_ADMIN_MODULE' ) and $fid > 0 and !in_array( $fid, $array_fam_view ) )
{
...
{
	$from_fid = array();
	foreach( $array_fam_view as $fid_i )
	{
		$from_fid[] = "r.listfid = '" . $fid_i . "'";
		$from_fid[] = "r.listfid like '" . $fid_i . ",%'";
		$from_fid[] = "r.listfid like '%," . $fid_i . ",%'";
		$from_fid[] = "r.listfid like '%," . $fid_i . "'";
	}
	$where .= ( empty( $where ) ) ? ' (' . implode( ' OR ', $from_fid ) . ')' : ' AND (' . implode( ' OR ', $from_fid ) . ')';
}
...
$db->select( 'r.id, r.fid, r.listfid, r.admin_id, r.author, r.title, r.alias, r.status , r.publtime, r.exptime, r.hitstotal, r.hitscm, u.username' )->order( 'r.' . $ordername . ' ' . $order )->limit( $per_page )->offset( ( $page - 1 ) * $per_page );
$result = $db->query( $db->sql() );

$data = $array_ids = array();
while( list( $id, $fid_i, $listfid, $post_id, $author, $title, $alias, $status, $publtime, $exptime, $hitstotal, $hitscm, $username ) = $result->fetch( 3 ) )
{
	...
	{
		$array_temp = explode( ',', $listfid );
		$check_edit = $check_del = 0;

		foreach( $array_temp as $fid_i )
		{
			if( isset( $array_fam_admin[$admin_id][$fid_i] ) )
			{
				...
				else
				{
					...
					elseif( $array_fam_admin[$admin_id][$fid_i]['pub_content'] == 1 and $status == 0 )
					{
						++$check_edit;
						$_permission_action['publtime'] = true;
						$_permission_action['re-published'] = true;
					...
					if( $array_fam_admin[$admin_id][$fid_i]['del_content'] == 1 )
					{
						++$check_del;
					}
					elseif( ( $status == 0 or $status == 4  or $status == 5 ) and $post_id == $admin_id )
...
$base_url_mod = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;per_page=' . $per_page . '&amp;fid=' . $fid . '&amp;stype=' . $stype . '&amp;q=' . $q . '&amp;checkss=' . $checkss;
...
foreach( $val_fam_content as $fam_content )
{
	$xtpl->assign( 'FAM_CONTENT', $fam_content );
	$xtpl->parse( 'main.fam_content' );
}
